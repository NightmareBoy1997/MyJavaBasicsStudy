...
public class BikeTest {
    ...
    private static int inputNumber(Scanner input) {
        ...
        while (true) {
            ...
            if (inputStr.length() != 1) {
                System.out.print("输入1位数字！你输得什么！重输：  ");
                continue;
            }
            ...
            if (flag) {
                System.out.print("数字！数字！数字！上过小学吗你！重输：  ");
                continue;
            }
    ...
    private static void into(Scanner input, BikeDB bikeDB) throws Exception {
        ...
        while (true) {
            ...
            if (no > 3) {
                throw new RuntimeException("你是喝醉酒了？，我们公司可没有这个，别来投放垃圾！");
            }
...
//            int bikeNumber = input.nextInt() ;
//            if(number > (10 - bikeNumber)){
//                throw new RuntimeException("别投了，别投了，求求了！装不下了！装不下了！");
...
//                    }
                ...
                System.out.println("添加成功！ 添加的单车是： \t" + bikes[i].getName() + "\t\t编号： " + bikes[i].getId());
...
class HaloBike extends Bike {
    ...
    public HaloBike(String string , boolean state , int dayMoney){
        this();
        this.setBorrowTime(string);
        this.setState(false);
        this.setDayMoney(dayMoney);
...
class MoBike extends Bike {
    ...
    public MoBike(String string , boolean state , int dayMoney){
        this();
        this.setBorrowTime(string);
        this.setState(false);
        this.setDayMoney(dayMoney);
